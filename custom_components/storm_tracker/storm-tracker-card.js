...
// ---------------------------------------------------------------------------
// Storm Tracker Info Card — sector activity list, sorted closest to farthest
// ---------------------------------------------------------------------------
  ...
  _sectorRows() {
    ...
    const rows = SECTOR_KEYS.map(key => {
      ...
      return {
        trend,
        dist,
        unit:  attrs.distance_unit  ?? 'mi',
        dir:   attrs.direction      ?? key.toUpperCase(),
        city:  attrs.nearest_city   ?? null,
        count: attrs.strike_count   ?? 0,
      };
    }).filter(Boolean);
  ...
  _render() {
    ...
    const rowsHtml = rows.length === 0
      ...
      : rows.map(r => {
          ...
          const strikes = r.count === 1 ? '1 strike' : `${escHtml(String(r.count))} strikes`;
          const cityHtml = r.city
            ? `<span class="city">Near ${escHtml(r.city)}</span>`
            : '';
          return `
            <div class="row">
              ...
              <div class="body">
                ...
                <div class="sub">${cityHtml}<span class="count">${strikes}</span></div>
              </div>
            </div>`;
        }).join('');
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .card-header {
          font-size: 1.05rem; font-weight: 600; padding: 0 0 10px;
          color: var(--primary-text-color); letter-spacing: 0.02em;
        }
        .empty { padding: 12px 0; color: var(--secondary-text-color); font-size: 0.9rem; }
        .row {
        ...
        .dist { font-size: 1rem; font-weight: 600; color: var(--primary-text-color); }
        .trend { font-size: 0.8rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
        .sub { display: flex; gap: 8px; margin-top: 2px; flex-wrap: wrap; }
        .city { font-size: 0.85rem; color: var(--secondary-text-color); }
        .count { font-size: 0.8rem; color: var(--disabled-text-color, rgba(255,255,255,0.4)); }
      </style>
      <ha-card>
        <div class="card-header">${escHtml(this._config.title)}</div>
        ${rowsHtml}
...
window.customCards.push({
  ...
  description: 'Active storm sectors sorted closest to farthest with nearest city',
  preview:     false,
  documentationURL: 'https://github.com/steve-dietzel/storm-tracker',
});
